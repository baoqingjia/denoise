...
sumspe=sumspe+spe{i};
 ...
 WritePath=['D:\topspin\data\400.3.2mm.20221116\9999\pdata\1\'];
...
disp(['SNR value: ', num2str(snr_value), ' dB']);
    ...
    [c,l] =wavedec(real(abs(sum(Phase,2)))',4,'db4');% wavedec((real(fftshift(fft(fidNoiseSum)))),4,'db4');
   ...
   DataCell1=real(NMR_denoised);
   
    WritePathplus= ['D:\topspin\data\400.3.2mm.20221116\5555\pdata\1\'];
     fileID = fopen([WritePathplus '1r'],'w','l');
   fwrite(fileID,round(round(DataCell1*10e12)/max(max(round(DataCell{1}*10e12)))*Max),'int32');
   fclose(fileID);