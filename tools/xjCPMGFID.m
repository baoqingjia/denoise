...
fidname{1,1}='E:\topspindata\data\400.3.2mm.20221116\2000\fid';
for m=1:1
    ...
    fid01=fid000;                               
    OneEchoPoint=204;                           %模拟回波中采样点数量       
    GuessTotalPoiint=OneEchoPoint*20;           %总回波数
...
%     fid01=Fid_noise_sup;
    ...
    figure;plot(abs(fidReshape))                %使用 plot 函数绘制了矩阵 fidReshape 中每一列数据的幅度，相当于把整个回波序列在时域上展开并可视化
    
    ProfileSpecAll=fftshift(fft((fidReshape),[],1),1);
    fidReshape=fidReshape-repmat(mean(fidReshape(end-200:end,:),1),[size(fidReshape,1) 1]);
    ...
    SpecAll=fftshift(fft((AllFidZeroFill),[],1),1);
     ...
     a=abs(sum(abs(SpecAll(:,1:10)),2));    %对前10列频谱进行取绝对值求和
%      a=flipud(a);
     b=abs(SpecAll(:,1));                   %第一列取绝对值
     a=a-mean(a(10:20));
     b=b-mean(b(10:20));
     a=a/max(a);
     b=b/max(b);
...
%     subplot(1,2,2);plot(abs(AverageSpecDenoise))
    ...
    figure;plot(real(AllFidZeroFill))  %绘制出以 AllFidZeroFill 的下标为横坐标、实部为纵坐标的折线图，以展示 AllFidZeroFill 向量的实部随下标变化的趋势。

    
    ImagShowNum=18
    figure('Name','all Spec');
    ha = tight_subplot(1,ImagShowNum,0.001,0.02,0.02);
    ...
    figure;subplot(1,2,1);plot(abs(sum(DenoisedSpecImag,2)));subplot(1,2,2);plot((abs(sum(SpecAll,2))))   %svd，原始
...
%     plot(abs(sum(SpecAll(:,1:1),2))/max(abs(sum(SpecAll(:,1:1),2))),'k');hold off;
    ...
    ImagShowNum=18
    figure('Name','all Spec');
    ha = tight_subplot(2,ImagShowNum,0.001);   %2行，第一行原图像，第二行svd
    for iSilce=1:ImagShowNum
        axes(ha(iSilce)); %#ok<LAXES>
        plot((abs((SpecAll(:,iSilce)))));ylim([0,max(abs(SpecAll(:)))])
    ...
    figure;plot(abs(DenoisedSpecImag(211,:)));hold on; plot(abs(SpecAll(211,:)),'r-*');hold on;  %绘制第211行的原始谱图和去噪谱图，并将它们以不同的线条颜色在同一张二维坐标图上展示，以便比较它们在不同频率下的幅度值
