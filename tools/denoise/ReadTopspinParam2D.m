function [paramValue] = ReadTopspinParam2D(dataPath,paramName)
...
% if(isempty(paramName) || isempty(fidPath))
    ...
    if exist([dirPath '/proc2s'],'file')
        procPath=[dirPath,'/proc2s'];
        index_dir=strfind(procPath,'/');
        str_temp=dirPath(1:index_dir(end-2)-1);
        acquPath=[str_temp,'/acqu2s'];
    ...
    if exist([dirPath '/acqu2s'],'file')
        acquPath=[dirPath,'/acqu2s'];
...
if(isempty(lenParamVal))
    lenParamVal=strfind(dataParsFiles((indParam+length(pName)):end),'##end');
end
