...
elseif strcmpi(xT,'median')
    xT = nanmedian(xP);
    note = 'median';
elseif strcmpi(xT,'average2')   %AVG2
    xT = MeanwNaN(xP);           %AVG2
    avg2_flag = 1;                        %AVG2
    if options(1) ~= 0;           %AVG2
        avg_power = input('which multiplier for the 2nd average? [default is 3] ');      %AVG2
    else avg_power = 3;      %AVG2
    end
    if isempty(avg_power); avg_power = 3;end                %AVG2
    note = 'average2';                 %AVG2
...
if flag_coshift
   ...
   [xP,nil,wint] = icoshift(xT,xP,'whole',n_co,[0 1 0]); %#ok<*ASGLU> %FRSA preprocessing COshift doesn't use NaNs
   ...
   if  strcmpi(note,'average2')
      xT = nanmean(xP); %FRSA uses the new CO-shifted average as a first iteration average target
   end
...
if flag % a Reference signal has been chosen
   ...
   % AVG2 secong iteration (FrSa)
   if avg2_flag
       for i = 1:size(allint,1)
           if options(1) ~= 0;
               if whole, fprintf('\n Co-shifting again the whole %g samples... \r',nP);
               else fprintf('\n Co-shifting again interval no. %g of %g... \r',i,size(allint,1));
               end
           end
%            intervalnow = xCS(:,allint(i,2):allint(i,3)); % (FrSa) further align the previously aligned matrix
           intervalnow = xP(:,allint(i,2):allint(i,3));  % align the raw matrix using the newly calculated average2 target
           target1  = mean(xCS(:,allint(i,2):allint(i,3)));
           min_interv=min(target1);
           target = (target1-min_interv).*avg_power; %AVG2 power multiplier and baseline correction
           missind     = ~all(isnan(intervalnow),2);
           if ~all(isnan(target)) && sum(missind) ~= 0
               [cosh_interval,loc_ind]              = coshifta(target,intervalnow(missind,:),0,n,[options(1:3),BLOCKSIZE]);
               xCS(missind,allint(i,2):allint(i,3)) = cosh_interval;
               xT(allint(i,2):allint(i,3)) = target;
               ind(missind,i)                       = loc_ind';
           else
               xCS(:,allint(i,2):allint(i,3)) = intervalnow;
           end
...
if options(1)==2
   ...
   if flag_coshift
      ...
      if avg2_flag %AVG2
          note = ['average2 * ', num2str(avg_power)];                 %AVG2
      end
      legend (h, note);
   else
      if any(strcmpi(note,{'average','median', 'max', 'average2'})) % MAX
         subplot(sp1,1,1)
         h = plot (Scal(x),xT(x), 'Color','r', 'LineWidth', 4);
         if avg2_flag %AVG2
             note = ['average2 * ', num2str(avg_power)];                 %AVG2
...
elseif strcmpi(n,'f')
   ...
   auto = 1; % switch for the fast automatic search on %FRSA modified from 1 to 0
   if rw ~=1
...
if auto == 1
   ...
   while auto == 1 % Automatic search for the best "n" for each interval
      ...
      if (max(abs(ind)) == n) && try_last ~= 1 %to check
         if n+src_step >= size(refW,2)
             try_last = 1; continue %FRSA to avoid n > interval size
         end
         n = n+src_step ;
         continue
      elseif (max(abs(ind)) < n) && n+src_step < size(refW,2) && try_last ~= 1
         n = n+src_step ;
         try_last = 1;
         continue
      else
