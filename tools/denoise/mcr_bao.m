function [mcrdata]=mcr_bao(pfgnmrdata,ncomp,Specrange,Options,Nug,Ginit)
...
% for i=1:size(Cbest,2)
%     if(max(abs(Cbest(:,i)))<0.5*max(abs(Cbest(:))))
%         Cbest(:,i)=0;
%     end
% end
DenoiseData=(Cbest*Sbest);
figure(17);subplot(1,2,1);plot(abs(Cbest(:,:)));
subplot(1,2,2);plot(abs(Sbest(:,:)'));



figure(18);subplot(1,2,1);plot(abs(X(8,:)'));
subplot(1,2,2);plot(abs(DenoiseData(8,:)'));

figure(19);subplot(1,2,1);plot(abs(sum(X,1)));
