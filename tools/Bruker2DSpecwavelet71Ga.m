...
FileDir='I:\topspindata\topspindata\ljw-20230425-2_HMQC_2\';
...
for it=1:43
 RealSpec2D{it}=abs(SpecAll2D(:,:,it));
%  RealSpec2D{it}=real(SpecAll2D(:,:,it));
end


 Realsum= abs(sum(SpecAll2D, 3));
 RealimgWaveletMultiframe = waveletMultiFrame(real(SpecAll2D), 'k', 8, 'p',10, 'windowSize',5, 'maxLevel',5, 'weightMode', 4, 'basis', 'dualTree');
 v2=max(max(abs(RealimgWaveletMultiframe(:,:))));
 v22=v2*(0.89.^(1:20));
 x=linspace(73.4,-51.7,4096);
 y=linspace(5148,-5115,128);
 [X,Y]=meshgrid(x,y);
...
figure; subplot(1,2,1); plot(abs(sum(RealimgWaveletMultiframe,1)));subplot(1,2,2);plot(abs(sum(Realsum,1))) ;

figure; subplot(1,2,1); plot(abs(RealimgWaveletMultiframe(:,70))); subplot(1,2,2);plot(abs(sum(Realsum,2))) ;
