function res = waveletSoftThreshold(img, varargin)
% RES = WAVELETSOFTTHRESHOLD(IMG, VARARGIN)
% Performs standard wavelet soft thresholding. Read your favorite wavelet
...
if strcmp(basis, 'dualtree')
    ...
    for l = 1:lmax
        for direction = 1:3
            ...
            y = max(abs(c) - mu, 0);
            y = y ./ (y + mu) .* c;
            w{l}{1}{direction} = real(y);
            w{l}{2}{direction} = imag(y);
        end
    end
...
elseif strcmp(basis, 'dddtcwt')
    ...
    for l = 1:lmax
        ...
        y = max(abs(horizontal{l} - mu), 0);
        horizontal{l} = y ./ (y + mu) .* horizontal{l};
        y = max(abs(vertical{l} - mu), 0);
        vertical{l} = y ./ (y + mu) .* vertical{l};
        y = max(abs(diagonal{l} - mu), 0);
        diagonal{l} = y ./ (y + mu) .* diagonal{l};
    end
