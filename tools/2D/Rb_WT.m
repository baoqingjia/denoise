...
v3= max(max(abs(denoisedMatrix(:,:))));
 ...
 v2= max(max(abs(Realsum(:,:))));
 v22=v3*(0.855.^(1:20));
 figure;contour(X,Y,Realsum',v22);
 figure; subplot(1,2,1); plot(sum(Realsum,2));subplot(1,2,2); plot(sum(denoisedMatrix,2))
