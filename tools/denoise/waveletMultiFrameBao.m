function imgRes = waveletMultiFrameBao(img, varargin)
...
% R=zeros(numFrames,maxLevel,numFrames-1);
% S=zeros(numFrames,maxLevel,numFrames-1);
% WR=zeros(numFrames,maxLevel);
% WS=zeros(numFrames,maxLevel);
for  level=1: maxLevel
    for num=1:numFrames
        if level == 1
            FrameWaveAppro1=imresize(img(:,:,num),size(Coeff(level).approx(:,:,num)));
        else
            %             FrameWaveAppro1= abs(Coeff(level-1).approx(:,:,num));
            FrameWaveAppro1=imresize( abs(Coeff(level-1).approx(:,:,num)),size(Coeff(level).approx(:,:,num)));
        end
        for other=1:(numFrames-1)
            if(other==num)
                continue;
            end
            if level == 1
                FrameWaveAppro2=imresize(img(:,:,other),size(Coeff(level).approx(:,:,other)));
            else
                FrameWaveAppro2= imresize( abs(Coeff(level-1).approx(:,:,other)),size(Coeff(level).approx(:,:,other)));
            end
            
%             TempA=corrcoef(FrameWaveAppro1,FrameWaveAppro2);
%             R(level).=TempA(2,1);
            
           R(level).approx(:,:,other,num)=PCCMap(FrameWaveAppro1,FrameWaveAppro2,2);
           [~, S(level).approx(:,:,other,num)]=ssim(FrameWaveAppro1,FrameWaveAppro2);
%              disp([R(num,level,other)    S(num,level,other)]);
        end


         WR(level).approx(:,:,num)=0.5*median(R(level).approx(:,:,:,num)+1,3);
         WS(level).approx(:,:,num)=median(  S(level).approx(:,:,:,num),3);
         W(level).approx(:,:,num)=0.5*(WR(level).approx(:,:,num)+WS(level).approx(:,:,num));
    end
end
...
for  level=1: maxLevel
    for num=1:numFrames  
        Coeff(level).horizontal(:,:,num) = Coeff(level).horizontal(:,:,num).* WS(level).approx(:,:,num);
        Coeff(level).vertical(:,:,num) = Coeff(level).vertical(:,:,num).* WS(level).approx(:,:,num);
        Coeff(level).diagonal(:,:,num) = Coeff(level).diagonal(:,:,num) .*WS(level).approx(:,:,num);
    end
end

% for  level=1: maxLevel
%     for num=1:numFrames  
%         Coeff(level).horizontal(:,:,num) =  wthresh(Coeff(level).horizontal(:,:,num),'s',median(abs(Coeff(level).horizontal(:,:,num)))/0.3);
%         Coeff(level).vertical(:,:,num) =  wthresh(Coeff(level).vertical(:,:,num),'s',median(abs(Coeff(level).vertical(:,:,num)))/0.3);
%         Coeff(level).diagonal(:,:,num) =  wthresh(Coeff(level).diagonal(:,:,num),'s',median(abs(Coeff(level).diagonal(:,:,num)))/0.3);
%     end
% end
