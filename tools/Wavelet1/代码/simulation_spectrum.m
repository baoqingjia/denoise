...
idealFid=SIMPSONread('E:\MATLAB\bin\PHIP_Dynamic_image\denoise\SimpsonData\27Al\27Al-1688368125.spe');

figure;plot(real(idealFid.Spectrum));
IdealSpecdata=idealFid.Spectrum;
IdealSpecdata=IdealSpecdata(7600-62:8500+61);
...
noiseLevel=[   0.45 0.35 0.25 ];

FreqScale=linspace(-idealFid.SweepWidthTD2/2,idealFid.SweepWidthTD2/2,length(IdealSpecdata)); %作为x轴输入数据
...
for k=1:length(noiseLevel)
    ...
    imgWaveletMultiframe = waveletMultiFrame(imgNoiseVol, 'k', 5, 'p', 8, 'windowSize',1,'maxLevel', 5, 'weightMode', 4, 'basis', 'dualTree','theta1');
...
figure;
subplot(2,3,1);plot((real(sum(imgWaveletMultiframe,2)))/32, 'color', [0.5, 0, 0.5]);box off;axis off;
%box off;xlim([1, 2048]);set(gca, 'YTick', []);  set(gca, 'YColor', 'none');  ylabel(''); 
subplot(2,3,2);plot((real(fftshift(fft(fidNoiseSum)))), 'color', [0.5, 0, 0.5]);box off;axis off; 
subplot(2,3,3);plot(real(IdealSpecdata), 'color', [0.5, 0, 0.5]); box off;axis off;
%subplot(2,3,4);plot(real(SpecDenoiseCzdzow));box off;axis off; 
subplot(2,3,4);plot(real(SpecDenoiseCzdzow), 'color', [0.5, 0, 0.5]);box off;axis off; 
subplot(2,3,5);plot(real(NMR_denoisedWavelet), 'color', [0.5, 0, 0.5]);box off;axis off; 
subplot(2,3,6); plot(real(NMR_denoised), 'color', [0.5, 0, 0.5]);box off;axis off; 
...
%MCR去噪
 pfgnmrdata.filename='E:\MATLAB\bin\PHIP_Dynamic_image';
